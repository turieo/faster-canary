...
import numpy as np
import torch
from nemo.collections.asr.modules.hybrid_decoder import HybridDecoder
...
class EncDecMultiTaskModel(ASRModel, ExportableEncDecModel, ASRBPEMixin, ASRModuleMixin, ASRTranscriptionMixin):
    ...
    def __init__(self, cfg: DictConfig, trainer: Trainer = None):
        ...
        self.hybrid_decoder = HybridDecoder(decoder_model=self.transf_decoder, classifier_model=self.log_softmax, tokenizer=self.tokenizer)
    ...
    def _transcribe_forward(
        ...
        # trick for timestamps
        tmp_return_hypotheses = trcfg.return_hypotheses
        trcfg.return_hypotheses = True
        logits, logits_len, greedy_predictions = self.timestamps_asr_model.forward(input_signal=audio, input_signal_length=audio_lens)
        ctc_hypotheses = self.timestamps_asr_model._transcribe_output_processing(dict(logits=logits, logits_len=logits_len), trcfg)
        del logits, logits_len, greedy_predictions
        trcfg.return_hypotheses = tmp_return_hypotheses
    ...
    def _transcribe_output_processing(self, outputs, trcfg: MultiTaskTranscriptionConfig) -> GenericTranscriptionType:
        ...
        ctc_hypotheses = outputs.pop('ctc_hypotheses')
        del log_probs
        ...
        reference_sequences  = [self.tokenizer.text_to_ids(hypothesis.text) for hypothesis in ctc_hypotheses]
